...
abstract contract BaseStrategy is IStrategy, Transfer, Errors {
    ...
    function registerRecipient(
        bytes memory _data,
        address _sender
    ) external payable onlyAllo onlyInitialized returns (address recipientId) {
    ...
    function distribute(
        ...
        bytes memory _data,
        address _sender
    ) external onlyAllo onlyInitialized {
    ...
    function getPayouts(
        address[] memory _recipientIds,
        bytes[] memory _data
    ) external view virtual override returns (PayoutSummary[] memory) {
