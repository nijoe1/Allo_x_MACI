...
export function getTallyResultProof(recipientIndex: number, recipientTreeDepth: number, tally: any) {
    ...
    const resultTree = new IncrementalQuinTree(recipientTreeDepth, BigInt(0), LEAVES_PER_NODE, hash5)
...
export function getTallyResultProofBatch(
    ...
    const proofs = [] as any
    const lastIndex = recipientStartIndex + batchSize > tallyCount ? tallyCount : recipientStartIndex + batchSize
...
export async function addTallyResultsBatch(
    ...
    const newTallyCommitment = hash3([newResultCommitment, spentVoiceCreditsHash, perVOSpentVoiceCreditsHash])
    ...
    for (let i = startIndex; i < tally.length; i = i + batchSize) {
        const proofs = getTallyResultProofBatch(i, recipientTreeDepth, tallyData, batchSize)
        ...
        for (let i = 0; i < proofs.length; i++) {
            ...
            // @ts-ignore
            encodedData.push((await QVMACI._addTallyResult.populateTransaction(data)).data)
        }
...
export { createMessage, getRecipientClaimData, genProofs, proveOnChain, verify, genLocalState }
