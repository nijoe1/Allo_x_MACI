...
contract MACIFactory is Ownable, Params, SnarkCommon, DomainObjs, MACICommon {
    ...
    constructor(address _vkRegistry, Factories memory _factories, address _verifier) {
     ...
     */
    function getMessageBatchSize(uint8 messageTreeSubDepth) public pure returns (uint256 _messageBatchSize) {
     ...
     */
    function setMaciParameters(uint8 _stateTreeDepth, TreeDepths calldata _treeDepths) public onlyOwner {
        uint256 messageBatchSize = getMessageBatchSize(_treeDepths.messageTreeSubDepth);

        if (
            !vkRegistry.hasProcessVk(
                _stateTreeDepth,
        ...
        if (!vkRegistry.hasTallyVk(_stateTreeDepth, _treeDepths.intStateTreeDepth, _treeDepths.voteOptionTreeDepth)) {
    ...
    function deployMaci(
        ...
        if (!vkRegistry.hasTallyVk(stateTreeDepth, treeDepths.intStateTreeDepth, treeDepths.voteOptionTreeDepth)) {
            revert TallyVkNotSet();
        }
