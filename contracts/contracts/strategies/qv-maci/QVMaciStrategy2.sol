...
contract QVMaciStrategy is BaseStrategy, Multicall, Events_Errors {
    ...
    MACI.PubKey public coordinatorPubKey;

    string public tallyHash;

    uint256 public totalTallyResults;

    IPollFactory public __fundingRoundFactory;

    IMessageProcessorFactory public __messageProcessorFactory;

    ITallySubsidyFactory public __tallyFactory;

    uint8 __stateTreeDepth;

    MACI.TreeDepths public treeDepths;

    uint8 public constant TREE_ARITY = 8;
    ...
    constructor(
        address registryIndexer,
        address _allo,
        string memory _name,
        IPollFactory _fundingRoundFactory,
        IMessageProcessorFactory _messageProcessorFactory,
        ITallySubsidyFactory _tallyFactory,
        uint8 _stateTreeDepth,
        MACI.TreeDepths memory _treeDepths,
        address _verifier,
        address _vkRegistry
    ) BaseStrategy(_allo, _name) {
        _registryIndexer = IRegistryIndexer(registryIndexer);
        __fundingRoundFactory = _fundingRoundFactory;
        __messageProcessorFactory = _messageProcessorFactory;
        __tallyFactory = _tallyFactory;
        __stateTreeDepth = _stateTreeDepth;
    ...
    function __QVBaseStrategy_init(uint256 _poolId, InitializeParams memory _params) internal {
        ...
        uint256 pollDuration = allocationEndTime - block.timestamp;

        _maci = new MACI(
            __fundingRoundFactory,
            __messageProcessorFactory,
            __tallyFactory,
            __tallyFactory,
            SignUpGatekeeper(address(this)),
            InitialVoiceCreditProxy(address(this)),
            TopupCredit(address(0)),
            __stateTreeDepth
        );
    ...
    /// @ _signUpGatekeeperData Data to pass to the sign-up gatekeeper's
    ///     register() function. For instance, the POAPGatekeeper or
    ///     SignUpTokenGatekeeper requires this value to be the ABI-encoded
    ///     token ID.
    /// @ _initialVoiceCreditProxyData Data to pass to the
    ///     InitialVoiceCreditProxy, which allows it to determine how many voice
    ///     credits this user should have.
    function signUp(MACI.PubKey memory _pubKey) public onlyBeforeAllocationEnds {
        if (isFinalized) {
            revert RoundAlreadyFinalized();
        }
        bytes memory _data = abi.encode(msg.sender, poolId);
        _maci.signUp(_pubKey, _data, _data);
    }
    ...
    function isTallied() private view returns (bool) {
        ...
        uint256 tallyBatchSize = uint256(TREE_ARITY) ** uint256(intStateTreeDepth);
    ...
    function finalize(
        ...
        if (isAddressZero(address(this))) revert MaciNotSet();
        ...
        if (!verified || _totalSpent != totalRecipientVotes) {
     ...
     */
    function isVerifiedUser(address _allocator) internal view returns (bool) {
    ...
    function getRecipient(uint256 _index) external view returns (address) {
        return recipientIndexToAddress[_index];
    }
